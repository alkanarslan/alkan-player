...
- The app supports local audio playback (MP3, FLAC, WAV, OGG, M4A, AAC, WMA, OPUS, AIFF), a 10-band equalizer, audio visualizer, YouTube downloading/streaming, YouTube Music browsing, Spotify playlist import, listening statistics dashboard, and random playlist generation.

## Architecture

| File | Role |
|---|---|
| `main.js` | Main process (~674 lines): BrowserWindow setup, IPC handlers, filesystem access, metadata parsing (incl. genre), YouTube/Spotify integrations. |
| `preload.js` | Context bridge (~52 lines): exposes `window.electronAPI` to the renderer via `contextBridge.exposeInMainWorld`. |
| `database.js` | SQLite persistence (~462 lines): library, playlists, listening history storage; dashboard statistics queries. |
| `src/renderer.js` | Renderer process (~4,670 lines): UI logic, playback, Web Audio API (EQ, visualizer, VU meter), i18n, state management, listen tracking, dashboard rendering, playlist management. |
| `src/index.html` | UI markup (~625 lines, frameless window with glassmorphism design). |
| `src/styles.css` | All styling (~3,680 lines), CSS custom properties for theming (6 themes), dashboard styles. |
| `ytmusic.js` | YouTube Music internal API client (browse, search, playlist, stream URL resolution). |

### Electron Process Boundary

- **Main process** (`main.js`, `preload.js`, `ytmusic.js`, `database.js`): Node.js APIs, filesystem, `child_process`, native dialogs, network requests, SQLite database.
...
- **SQLite database** (`better-sqlite3`) at `%APPDATA%/alkan-player/alkan-player.db`.
- Tables: `library`, `playlists`, `playlist_tracks`, `listening_history`.
- Database functions live in `database.js` and are called from `main.js` IPC handlers.
- The renderer persists data via `window.electronAPI.saveLibrary()`, `saveSettings()`, `savePlaylists()`, `saveListenEvent()`.

### Listening Statistics

- Listen events are tracked in the renderer (`state._listenStart`, `state._listenedMs`, `state._listenTrack`).
- Events are saved when: track changes, playback ends, or the app closes (`beforeunload`).
- Only events with ≥15 seconds of listening are persisted (prevents skips from counting).
- `getDashboardStats()` in `database.js` returns: overall stats, top artists/tracks/albums/genres, format distribution, daily activity (30 days), hourly distribution, weekly comparison, and recent plays.

### Playlist System

- Playlists are stored as `[{id, name, tracks: [filePath]}]` in `state.playlists`.
- The client-side `id` is generated by `generateId()` and is NOT persisted in the database; it is reassigned on load via `loadPlaylists()`.
- Random Mix creates a 24-track playlist from shuffled library tracks, limiting each artist to max 4 songs.
- Playlist names can be renamed via a modal triggered from the card title or the edit button.
...
- **Single quotes** in main process files (`main.js`, `preload.js`, `ytmusic.js`, `database.js`).
...
- **IPC channel names**: `kebab-case` — e.g. `'window-minimize'`, `'open-files'`, `'get-metadata'`, `'save-listen-event'`, `'get-dashboard-stats'`.
- **State properties**: `camelCase` in the global `state` object — e.g. `state.currentTrack`, `state.librarySort`.
- **Database tables/columns**: `snake_case` — e.g. `listening_history`, `played_at`, `listened_duration`.

### Imports / Requires

- Group requires at the top of the file in this order:
  1. Electron modules (`electron`, `path`, `fs`)
  2. Third-party packages (`music-metadata`, `node-id3`, `better-sqlite3`, etc.)
  3. Local modules (`./ytmusic`, `./database`)
...
- Listen tracking state: `state._listenStart`, `state._listenedMs`, `state._listenTrack`.
...
- Translation keys use dot notation: `'nav.library'`, `'settings.theme'`, `'ytmusic.searchPlaceholder'`, `'dash.totalListening'`, `'playlist.randomMix'`.
- All new user-facing strings must be added to all 4 language blocks.
...
- `better-sqlite3` — SQLite database for library, playlists, and listening history.
- `ffmpeg-static` — FFmpeg binary for audio conversion.
- `music-metadata` — Parses audio file metadata (title, artist, album, genre, cover art).
