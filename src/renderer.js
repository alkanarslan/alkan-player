...
function initAudioContext() {
  ...
  stereoPanner.pan.value = state.settings.balance || 0;
}
...
const translations = {
  tr: {
    ...
    "ytmusic.loginDesc":
      "Kişiselleştirilmiş önerilerinizi görmek için Google hesabınızla giriş yapın",
    ...
    "ytmusic.noRecommendations":
      "Öneri bulunamadı. Sayfayı yenilemeyi deneyin.",
    ...
    "settings.downloadDesc":
      "YouTube'dan indirilen müziklerin kaydedileceği klasör",
    "settings.data": "Veri Dosyaları",
    "settings.dataPath": "Veri Klasörü",
    "settings.dataDesc":
      "Kütüphane ve çalma listesi verilerinin saklandığı klasör",
    "settings.library": "Kütüphane",
    "settings.libraryCount": "Kütüphanedeki Şarkı Sayısı",
    "settings.clearLibrary": "Kütüphaneyi Temizle",
    "settings.clearLibraryDesc":
      "Tüm şarkıları kütüphaneden kaldır (dosyalar silinmez)",
    ...
    "toast.ytmusicNotLogged":
      "Henüz giriş yapılmadı. Lütfen Google hesabınızla giriş yapın.",
    ...
    "modal.clearLibraryConfirm":
      "Tüm şarkılar kütüphaneden kaldırılacak. Dosyalar silinmez. Devam etmek istiyor musunuz?",
    "spotify.importTitle": "Spotify İçe Aktar",
  ...
  en: {
    ...
    "ytmusic.loginDesc":
      "Sign in with your Google account to see personalized recommendations",
    ...
    "settings.clearLibraryDesc":
      "Remove all songs from the library (files are not deleted)",
    ...
    "toast.ytmusicNotLogged":
      "Not logged in yet. Please sign in with your Google account.",
  ...
  ar: {
    ...
    "settings.clearLibraryDesc":
      "إزالة جميع الأغاني من المكتبة (لن تُحذف الملفات)",
    "settings.about": "حول",
    ...
    "toast.ytmusicNotLogged":
      "لم يتم تسجيل الدخول بعد. الرجاء تسجيل الدخول بحساب Google.",
  ...
  it: {
    ...
    "youtube.downloadingLong":
      "Download in corso... potrebbe richiedere un po' di tempo.",
    "youtube.downloaded": "Brani scaricati",
    "youtube.noDownloads": "Nessun download",
    "ytmusic.loginTitle": "Connetti a YouTube Music",
    "ytmusic.loginDesc":
      "Accedi con il tuo account Google per vedere consigli personalizzati",
    ...
    "settings.downloadDesc":
      "Cartella in cui vengono salvati i download da YouTube",
    "settings.data": "File dati",
    "settings.dataPath": "Cartella dati",
    "settings.dataDesc":
      "Cartella in cui sono salvati i dati di libreria e playlist",
    "settings.library": "Libreria",
    "settings.libraryCount": "Brani in libreria",
    "settings.clearLibrary": "Svuota libreria",
    "settings.clearLibraryDesc":
      "Rimuovi tutti i brani dalla libreria (i file non verranno eliminati)",
    ...
    "toast.addedSongs": ({ count }) =>
      `Aggiunti ${count} ${count === 1 ? "brano" : "brani"}`,
    ...
    "toast.playbackError": ({ message }) =>
      `Errore di riproduzione: ${message}`,
    ...
    "toast.ytmusicNotLogged":
      "Non hai ancora effettuato l'accesso. Accedi con il tuo account Google.",
...
async function addFilesToLibrary(filePaths) {
  ...
  for (const fp of filePaths) {
    ...
    state.library.push({ filePath: fp, ...meta, coverArt: normalizedCover });
    added++;
  }
  renderTrackList();
  if (added > 0) {
...
async function loadLibrary() {
  ...
  for (let i = 0; i < state.library.length; i++) {
    if (!state.library[i].coverArt) {
      try {
        const meta = await window.electronAPI.getMetadata(
          state.library[i].filePath,
        );
        const normalizedCover = normalizeCoverArt(meta?.coverArt);
        if (normalizedCover) {
...
function setMarqueeTitle(text) {
  ...
  els.trackTitle.appendChild(span);
  requestAnimationFrame(() => {
    const needs = span.offsetWidth > els.trackTitle.clientWidth + 2;
    els.trackTitle.classList.toggle("marquee", needs);
  });
}

function normalizeCoverArt(value) {
  if (!value) return null;
  if (typeof value === "string") {
    if (value.startsWith("data:")) {
      const parts = value.split(",");
      if (parts.length > 1 && /^[0-9,\s]+$/.test(parts[1])) {
        const bytes = parts[1]
          .split(",")
...
async function playTrack(index, trackList = null) {
  ...
  if (!track.coverArt) {
    window.electronAPI
      .getMetadata(track.filePath)
      .then((meta) => {
        const normalized = normalizeCoverArt(meta?.coverArt);
        if (normalized) {
          track.coverArt = normalized;
          if (state.currentTrack?.filePath === track.filePath) {
            state.currentTrack.coverArt = normalized;
            updateNowPlaying();
          }
        }
      })
...
function updateNowPlaying() {
  ...
  const normalizedCover = normalizeCoverArt(track.coverArt);
  if (normalizedCover && normalizedCover !== track.coverArt) {
    track.coverArt = normalizedCover;
  }
...
els.volumeKnobWrapper?.addEventListener(
  "wheel",
  (e) => {
    e.preventDefault();
...
function setBalance(value) {
  state.settings.balance = value;
  if (stereoPanner) stereoPanner.pan.value = value;
  if (els.balanceSlider) els.balanceSlider.value = Math.round(value * 100);
  if (els.balanceValue) {
    const pct = Math.round(Math.abs(value) * 100);
    els.balanceValue.textContent =
      value === 0 ? "0%" : value < 0 ? `L ${pct}%` : `R ${pct}%`;
  }
  saveSettingsToFile();
}
...
els.balanceReset?.addEventListener("click", () => {
  setBalance(0);
});
...
els.shuffleBtn.addEventListener("click", () => {
  ...
  showToast(
    state.shuffle ? t("toast.shuffleOn") : t("toast.shuffleOff"),
    "info",
  );
});
...
$$(".context-item").forEach((item) => {
  item.addEventListener("click", () => {
    ...
    if (action === "play") {
      ...
      state.library = state.library.filter(
        (t) => t.filePath !== contextTarget.filePath,
      );
      renderTrackList();
      saveLibrary();
...
function showPlaylistPicker(filePath) {
  ...
  const listItems = state.playlists
    .map(
      (p, i) =>
  ...
  overlay.querySelectorAll(".playlist-pick-item").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (!state.playlists[idx].tracks.includes(filePath)) {
        ...
        savePlaylists();
        showToast(
          t("toast.addedToPlaylist", { name: state.playlists[idx].name }),
          "success",
        );
      } else {
...
function getPlaylistCover(playlist) {
  ...
  for (const fp of playlist.tracks) {
    ...
    if (track?.coverArt) {
      const normalized = normalizeCoverArt(track.coverArt);
      if (normalized && normalized !== track.coverArt)
        track.coverArt = normalized;
...
async function openPlaylistDetail(playlistId) {
  ...
  els.playlistTrackCount.textContent = t("common.songCount", {
  ...
  for (const fp of playlist.tracks) {
    ...
    if (!track) {
      try {
        const meta = await window.electronAPI.getMetadata(fp);
        const normalizedCover = normalizeCoverArt(meta?.coverArt);
        track = { filePath: fp, ...meta, coverArt: normalizedCover };
        state.library.push(track);
        addedToLibrary = true;
      } catch (err) {
        track = {
      ...
      if (removeBtn) {
        ...
        if (playlist && fp) {
          ...
          if (state.playQueue && state.activePlaylistId === playlist.id) {
            ...
            state.currentIndex = updated.findIndex(
              (t) => t.filePath === state.currentTrack?.filePath,
            );
          }
        }
        return;
...
$("#btn-new-playlist").addEventListener("click", () => {
  showModal(
    t("modal.newPlaylistTitle"),
    t("modal.newPlaylistPlaceholder"),
    (name) => {
      if (!name.trim()) return;
...
$("#btn-youtube-download").addEventListener("click", async () => {
  ...
  els.downloadStatus.querySelector("span").textContent = t(
    "youtube.downloadingLong",
  );
...
let spotifyImportTracks = [];
let spotifyImportAbort = false;

$("#btn-spotify-import")?.addEventListener("click", async () => {
  const url = $("#spotify-url")?.value.trim();
  if (!url) {
    showToast(t("toast.spotifyUrlMissing") || "Spotify linki girin", "error");
    return;
  }
  if (!url.match(/spotify\.com\/playlist\//)) {
    showToast(
      t("toast.spotifyUrlInvalid") || "Geçersiz Spotify playlist linki",
      "error",
    );
    return;
  }
  ...
  const statusText = $("#spotify-status-text");
  const tracksEl = $("#spotify-tracks");
  const trackList = $("#spotify-track-list");

  statusEl.classList.remove("hidden");
  tracksEl.classList.add("hidden");
  statusText.textContent = t("spotify.fetching") || "Playlist alınıyor...";

  const result = await window.electronAPI.spotifyGetPlaylist(url);
  statusEl.classList.add("hidden");

  if (!result.success) {
    showToast(result.error || "Playlist alınamadı", "error");
    return;
  }

  spotifyImportTracks = result.tracks.map((t, i) => ({
    ...t,
    index: i,
    searchQuery: `${t.title} ${t.artist}`.trim(),
    status: "pending", // pending, searching, found, downloading, done, error
    videoId: null,
    ytTitle: "",
  }));

  $("#spotify-tracks-count").textContent =
    `${result.title} — ${spotifyImportTracks.length} ${t("common.track") || "şarkı"}`;

  renderSpotifyTracks();
  tracksEl.classList.remove("hidden");
  showToast(`${spotifyImportTracks.length} şarkı bulundu`, "success");
});
...
function renderSpotifyTracks() {
  ...
  trackList.innerHTML = spotifyImportTracks
    .map((tr, i) => {
      const statusIcon =
        {
          pending: '<span class="sp-status sp-pending">--</span>',
          searching:
            '<span class="sp-status sp-searching"><div class="download-spinner" style="width:14px;height:14px"></div></span>',
          found: '<span class="sp-status sp-found">OK</span>',
          downloading:
            '<span class="sp-status sp-downloading"><div class="download-spinner" style="width:14px;height:14px"></div></span>',
          done: '<span class="sp-status sp-done">&#10003;</span>',
          error: '<span class="sp-status sp-error">&#10007;</span>',
        }[tr.status] || "";

      return `
      <div class="spotify-track-row" data-index="${i}">
        <span class="sp-num">${i + 1}</span>
        <div class="sp-info">
          <div class="sp-title">${escapeHtml(tr.title)}</div>
          <div class="sp-artist">${escapeHtml(tr.artist)}</div>
        </div>
        <div class="sp-yt-match">${tr.ytTitle ? escapeHtml(tr.ytTitle) : ""}</div>
        ${statusIcon}
        <button class="btn-ghost btn-sm sp-dl-btn" data-sp-index="${i}"
                ${tr.status === "done" || tr.status === "downloading" ? "disabled" : ""}>
  ...
  trackList.querySelectorAll(".sp-dl-btn").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
      const idx = parseInt(btn.dataset.spIndex);
      await downloadSpotifyTrack(idx);
    });
  });
}
...
async function searchYouTubeForTrack(track) {
  ...
  renderSpotifyTracks();
  const result = await window.electronAPI.youtubeSearch(track.searchQuery);
  if (result.success) {
    track.videoId = result.videoId;
    track.ytTitle = result.title;
    track.ytUrl = result.url;
    track.status = "found";
  } else {
    track.status = "error";
  }
...
async function downloadSpotifyTrack(index) {
  const track = spotifyImportTracks[index];
  if (!track || track.status === "done" || track.status === "downloading")
    return;

  // First search on YouTube if not found yet
  if (!track.videoId) {
    const found = await searchYouTubeForTrack(track);
    if (!found) {
      showToast(`"${track.title}" YouTube'da bulunamadı`, "error");
      return;
    }
  }
  ...
  const result = await window.electronAPI.youtubeDownload(track.ytUrl);
  if (result.success) {
    track.status = "done";
    state.downloads.push(result);
    renderDownloads();
    ...
    track.status = "error";
    showToast(`"${track.title}" indirilemedi`, "error");
  }
  renderSpotifyTracks();
}

$("#btn-spotify-download-all")?.addEventListener("click", async () => {
  spotifyImportAbort = false;
  const pending = spotifyImportTracks.filter((t) => t.status !== "done");
  if (pending.length === 0) {
    showToast(
      t("toast.spotifyAllDone") || "Tüm şarkılar zaten indirildi",
      "info",
    );
    return;
  }

  showToast(`${pending.length} şarkı indiriliyor...`, "info");
  const statusText = $("#spotify-status-text");
  const statusEl = $("#spotify-status");

  for (let i = 0; i < spotifyImportTracks.length; i++) {
    if (spotifyImportAbort) break;
    const track = spotifyImportTracks[i];
    if (track.status === "done") continue;

    statusEl.classList.remove("hidden");
    statusText.textContent = `${i + 1}/${spotifyImportTracks.length}: ${track.title}`;
  ...
  statusEl.classList.add("hidden");
  const doneCount = spotifyImportTracks.filter(
    (t) => t.status === "done",
  ).length;
  showToast(
    `${doneCount}/${spotifyImportTracks.length} şarkı indirildi`,
    "success",
  );
});
...
async function loadYtMusicHome() {
  ...
  shelvesEl.innerHTML = data.shelves
    .map(
      (shelf) => `
  ...
  container.querySelectorAll(".ytmusic-card-btn").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      if (action === "stream") {
        ...
        if (result.success) {
          showToast(
            t("toast.downloadSuccess", { title: result.title }),
            "success",
          );
          state.downloads.push(result);
          renderDownloads();
...
async function openYtPlaylistDetail(browseId, title) {
  ...
  $("#ytmusic-pl-tracks").innerHTML =
    `<div class="ytmusic-loading"><div class="download-spinner"></div><span>${t("ytmusic.loadingTracks")}</span></div>`;

  const data = await window.electronAPI.ytMusicGetPlaylist(browseId);

  if (data.error) {
    $("#ytmusic-pl-tracks").innerHTML =
      `<div class="ytmusic-empty">${t("ytmusic.loadFailed", { error: data.error })}</div>`;
    return;
  }
  ...
  if (data.thumbnail)
    $("#ytmusic-pl-thumb").innerHTML = `<img src="${data.thumbnail}" alt="">`;

  if (!data.tracks || data.tracks.length === 0) {
    $("#ytmusic-pl-tracks").innerHTML =
      `<div class="ytmusic-empty">${t("ytmusic.emptyPlaylist")}</div>`;
    return;
  }

  const tracksEl = $("#ytmusic-pl-tracks");
  tracksEl.innerHTML = data.tracks
    .map((t, i) => renderYtTrackItem(t, i))
    .join("");
    ...
    if (els.eqPresetChips) {
      els.eqPresetChips.querySelectorAll(".eq-chip").forEach((btn) => {
        btn.classList.toggle(
          "active",
          matched && btn.dataset.preset === matched,
        );
      });
    }
    scheduleEqSave();
...
function startVisualizer() {
  ...
  function draw() {
    ...
    for (let i = 0; i < barCount; i++) {
      ...
      ctx.roundRect(x, h - barHeight, barWidth - gap, barHeight, [
        radius,
        radius,
        0,
        0,
...
function startVuMeter() {
  ...
  function drawTascamMeter(ctx, x, y, w, h, level, peakLed, label) {
    ...
    const marks = [
      { v: 0.0, lbl: "-20", major: true, zone: "green" },
      ...
      { v: 0.7, lbl: "0", major: true, zone: "yellow" },
    ...
    for (const m of marks) {
      ...
      if (m.lbl) {
        ...
        ctx.font = m.major
          ? 'bold 9.5px "Courier New", monospace'
          : '7.5px "Courier New", monospace';
        ctx.fillStyle = col;
  ...
  function vuDraw() {
    ...
    if (vuLevelL > vuPeakHoldL) {
      vuPeakHoldL = vuLevelL;
      vuPeakTimerL = 0;
    } else {
      vuPeakTimerL++;
      if (vuPeakTimerL > 30) vuPeakHoldL -= 0.02;
    }
    if (vuLevelR > vuPeakHoldR) {
      vuPeakHoldR = vuLevelR;
      vuPeakTimerR = 0;
    } else {
      vuPeakTimerR++;
      if (vuPeakTimerR > 30) vuPeakHoldR -= 0.02;
    }
...
async function loadSettings() {
  ...
  if (!state.settings.eqPreset) {
    state.settings.eqPreset = "flat";
    changed = true;
  }
  if (!Array.isArray(state.settings.eqGains)) {
    state.settings.eqGains = [];
    changed = true;
  }
  if (typeof state.settings.balance !== "number") {
    state.settings.balance = 0;
    changed = true;
  }
  if (changed) saveSettingsToFile();
...
function renderSettings() {
  ...
  if (pathEl)
    pathEl.textContent = state.settings.downloadPath || t("common.default");
  if (dataPathEl) {
  ...
  if (countEl)
    countEl.textContent = t("common.songCount", {
...
async function init() {
  ...
  if (els.balanceSlider)
    els.balanceSlider.value = Math.round((state.settings.balance || 0) * 100);
  if (els.balanceValue) {
    const val = state.settings.balance || 0;
    const pct = Math.round(Math.abs(val) * 100);
    els.balanceValue.textContent =
      val === 0 ? "0%" : val < 0 ? `L ${pct}%` : `R ${pct}%`;
  }
  await loadLibrary();
  await loadPlaylists();
  renderDownloads();
  renderTrackList();
  applyLibraryView();
  const savedGains = Array.isArray(state.settings.eqGains)
    ? normalizeGains([...state.settings.eqGains])
    : [];
  const sliderCount = $$(".eq-slider").length;
  if (savedGains.length === sliderCount) {
    state.settings.eqGains = savedGains;
    $$(".eq-slider").forEach((slider, i) => {
